  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative w-[600px] h-[600px]">
        ...
        {/* Radar Sweep */}
        <div 
          className="absolute inset-0 rounded-full bg-gradient-to-r from-transparent via-transparent to-cyan-500/30"
          style={{ 
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="w-12 h-12 bg-cyan-500 rounded-full flex items-center justify-center active-glow animate-pulse">
            <i className="fa-solid fa-ship text-white text-lg"></i>
          </div>
        </div>
      </div>
    </div>
...
const VesselPoint: React.FC<{ x: number; y: number; name: string; status: 'normal' | 'critical' }> = ({ x, y, name, status }) => (
  <div 
    ...
    style={{ left: x, top: y }}
  >
    <div className={`w-3 h-3 rounded-full ${status === 'critical' ? 'bg-red-500 animate-ping' : 'bg-cyan-400 animate-pulse'}`} />
    <div className={`absolute top-0 left-0 w-3 h-3 rounded-full ${status === 'critical' ? 'bg-red-500' : 'bg-cyan-400'}`} />
    
    <div className="absolute left-5 top-0 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 border border-slate-700 px-2 py-1 rounded text-[10px] whitespace-nowrap z-50">
      <span className="font-bold text-white uppercase">{name}</span>
      <p className="text-slate-400">Class: Tanker</p>
    </div>
  </div>
);
